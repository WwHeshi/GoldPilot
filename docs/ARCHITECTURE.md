...
**GoldPilot** 是一款面向全球贵金属市场的**企业级智能分析平台**，采用**多智能体协同架构（Multi-Agent Collaboration Architecture）**，融合大语言模型（LLM）与实时金融数据，为投资者提供全方位的黄金市场洞察与决策支持。
...
│   ──────          ──────            ─────             ──────              │
│                                                                             │
│   React 18    ┌── FastAPI ──┐   ┌── LLM接口 ──┐   ┌── MySQL 8.0 ──┐      │
│   TypeScript  │   Python 3.11 │   │ OpenAI兼容 │   │   主存储       │      │
│   TailwindCSS │   Uvicorn     │   │  可配模型   │   │   事务处理     │      │
│   Vite        │   Pydantic    │   │  RAG引擎    │   └───────────────┘      │
...
GoldPilot 采用经典的**四层架构模式**，确保系统的可扩展性、可维护性与高可用性：
...
│                                                                             │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐   │
│  │ OpenAI兼容接口 │  │ 可选WebSearch │  │   腾讯财经    │  │   RSS聚合    │   │
│  │  (页面配置)   │  │   (工具调用)  │  │   (实时行情)  │  │   (新闻源)    │   │
│  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘   │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
GoldPilot 的核心创新在于其**多智能体协同决策机制**，各智能体通过**消息总线**进行异步通信，实现分布式智能推理：
...
GoldPilot 采用**三级缓存策略**，在数据新鲜度与系统性能之间取得最佳平衡：
...
│  ─────────────────────────────────────────                                   │
│  • 响应时间: 500ms ~ 3000ms                                                 │
│  • 数据源: OpenAI兼容模型、可选Web Search、腾讯财经                         │
│  • 限流策略: 令牌桶算法                                                      │
│  • 降级方案: 缓存数据 + 提示信息                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
class SecureConfig:
    ...
    def ai_api_key(self) -> str:
        """页面配置的 OpenAI 兼容接口密钥"""
        return load_from_database('ai_configs.api_key')
...
| 问题反馈 | 提交问题或建议 | [GitHub Issues](https://github.com/WwHeshi/GoldPilot/issues) |
| 项目主页 | GitHub 仓库 | [github.com/WwHeshi/GoldPilot](https://github.com/WwHeshi/GoldPilot) |

---

<p align="center">
  <strong>GoldPilot</strong> - 智能黄金投资，洞察财富先机
  <br>
